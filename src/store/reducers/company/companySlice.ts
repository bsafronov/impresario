...
import { ICompanies, ICompany, ICompanyMoney, ICompanyTransferMoney } from "./company.interface";
...
export const companySlice = createSlice({
  ...
  reducers: {
    ...
    transferMoney(state, action: PayloadAction<ICompanyTransferMoney>) {
      if (action.payload.object === 'free money' && action.payload.type === 'to') {
        state.freeMoney += action.payload.amount
      }

      if (action.payload.object === 'free money' && action.payload.type === 'from') {
        state.freeMoney -= action.payload.amount
      }
      ...
      if (action.payload.object === 'company' && action.payload.type === 'from') {
        state.companies = state.companies.map(company => {
          if (company.id === action.payload.companyId) {
            company.balance -= action.payload.amount
          }
